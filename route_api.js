if (Meteor.isServer) {
    function api_key_in_use(api_key){
        ...
        console.log("IN USE?")
        var user = Meteor.users.find({'api.api_id': api_key});
        if (user){
    ...
    function api_auth(api_key){
        console.log(api_key);
        var user = Meteor.users.findOne({'api.api_id': api_key});
        console.log(user);
        if (user){
            if (user.hasOwnProperty('api')){
                if (user.api.confirmed){
...
Router.route('connect', {
    ...
    if (json_data){
        if (json_data['email'] && json_data['api_key']){
            user = Meteor.users.findOne({'profile.email': json_data['email']});
                    ...
                    if (user.api.confirmed){
                        ...
                        if (!api_key_in_use()){
                            Meteor.users.update(user._id,{
                                $set:{'api' : {'api_id' : api_key, 'confirmed': false }}
                            });
...
.post(function(){
    ...
    if (user){
        if (user.roles == 'admin'){
            var places = Places.find({},{sort:{'votes': -1}}).fetch();
            for(var p in places){
              // console.log(p);
